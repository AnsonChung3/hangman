...
async def hangman(request):
    body = await request.json()
    index = body['index']
    if index == 'python':
        answer = 'python'
    else:
        answer = list(complete_list[index])
    guess = body['guess']
    result = []
...
async def new_man(request):
    index = random.randrange(0, len(complete_list))
